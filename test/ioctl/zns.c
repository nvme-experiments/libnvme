...
static void test_zns_append(void)
{
	...
	err = nvme_zns_append(test_link, TEST_NSID, zslba, nlb,
			      control, cev, dspec,
...
static void test_zns_report_zones(void)
{
	...
	arbitrary(&expected_data, sizeof(expected_data));
	set_mock_io_cmds(&mock_io_cmd, 1);
	err = nvme_zns_report_zones(test_link, TEST_NSID, TEST_SLBA,
				    opts,
				    extended, partial,
				    &data, sizeof(data),
				    &result);
...
static void test_zns_mgmt_send(void)
{
	...
	struct mock_cmd mock_io_cmd = {
		...
		.cdw11 = slba >> 32,
		.cdw13 = (zsaso << 9) | (!!select_all << 8) | (zsa << 0),
		.data_len = sizeof(expected_data),
		.out_data = &expected_data,
	};
	...
	arbitrary(&expected_data, sizeof(expected_data));
	set_mock_io_cmds(&mock_io_cmd, 1);
	err = nvme_zns_mgmt_send(test_link, TEST_NSID, slba, zsa, select_all,
				 zsaso, 0, data,
				 sizeof(data), &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);

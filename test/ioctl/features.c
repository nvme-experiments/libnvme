...
static void test_set_features(void)
{
	...
	err = nvme_set_features(test_link, TEST_NSID, TEST_FID, true, TEST_CDW11, TEST_CDW12, TEST_CDW13,
				TEST_CDW15, TEST_UUIDX, data, sizeof(data),
				&result);
	end_mock_cmds();
...
static void test_set_features_data(void)
{
	...
	err = nvme_set_features_data(
		test_link, TEST_NSID, TEST_FID, false, TEST_CDW11,
...
static void test_set_features_simple(void)
{
	...
	err = nvme_set_features_simple(
		test_link, TEST_NSID, TEST_FID, true, TEST_CDW11, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_arbitration(void)
{
	...
	err = nvme_set_features_arbitration(
		test_link, false, AB, LPW, MPW, HPW, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_arbitration(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_arbitration(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_power_mgmt(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_power_mgmt(test_link, true, PS, WH, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_power_mgmt(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_power_mgmt(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_lba_range(void)
{
	...
	err = nvme_set_features_lba_range(
		test_link, TEST_NSID, false, NUM, &range_types, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_temp_thresh(void)
{
	...
	err = nvme_set_features_temp_thresh(
		test_link, true, TMPTH, TMPSEL, THSEL, 0, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_temp_thresh(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_temp_thresh(test_link, TEST_SEL, 0, 0, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_err_recovery(void)
{
	...
	err = nvme_set_features_err_recovery(
		test_link, TEST_NSID, false, TLER, true, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_volatile_wc(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_volatile_wc(test_link, true, true, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_volatile_wc(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_volatile_wc(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_num_queues(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_num_queues(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_irq_coalesce(void)
{
	...
	err = nvme_set_features_irq_coalesce(
		test_link, false, THR, TIME, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_irq_coalesce(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_irq_coalesce(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_irq_config(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_irq_config(test_link, true, IV, true, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_write_atomic(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_write_atomic(test_link, false, true, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_write_atomic(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_write_atomic(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_async_event(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_async_event(test_link, true, EVENTS, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_async_event(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_async_event(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_auto_pst(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_auto_pst(test_link, false, true, &apst, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_auto_pst(void)
{
	...
	arbitrary(&apst, sizeof(apst));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_auto_pst(test_link, TEST_SEL, &get_apst, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_host_mem_buf(void)
{
	...
	err = nvme_get_features_host_mem_buf(
		test_link, TEST_SEL, &get_attrs, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_timestamp(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_timestamp(test_link, TEST_SEL, &get_ts);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
...
static void test_get_kato(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_kato(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_hctm(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_hctm(test_link, false, TMT2, TMT1, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_hctm(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_hctm(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_nopsc(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_nopsc(test_link, true, true, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_nopsc(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_nopsc(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_rrl(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_rrl(test_link, false, NVMSETID, RRL, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_rrl(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_rrl(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_plm_config(void)
{
	...
	err = nvme_get_features_plm_config(
		test_link, TEST_SEL, NVMSETID, &get_config, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_plm_window(void)
{
	...
	err = nvme_set_features_plm_window(
		test_link, false, NVMSETID, SEL, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_plm_window(void)
{
	...
	err = nvme_get_features_plm_window(
		test_link, TEST_SEL, NVMSETID, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_lba_sts_interval(void)
{
	...
	err = nvme_set_features_lba_sts_interval(
		test_link, true, LSIRI, LSIPI, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_lba_sts_interval(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_lba_sts_interval(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_host_behavior(void)
{
	...
	err = nvme_get_features_host_behavior(
		test_link, TEST_SEL, &get_behavior, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_sanitize(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_sanitize(test_link, false, true, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_sanitize(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_sanitize(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_endurance_evt_cfg(void)
{
	...
	err = nvme_set_features_endurance_evt_cfg(
		test_link, true, ENDGID, EGWARN, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_endurance_event_cfg(void)
{
	...
	err = nvme_get_features_endurance_event_cfg(
		test_link, TEST_SEL, ENDGID, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_iocs_profile(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_iocs_profile(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_sw_progress(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_sw_progress(test_link, true, PBSLC, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_get_sw_progress(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_sw_progress(test_link, TEST_SEL, &result);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_host_id(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_host_id(test_link, true, false, hostid);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
}
...
static void test_set_host_id_extended(void)
{
	...
	arbitrary(hostid, sizeof(hostid));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_host_id(test_link, false, true, hostid);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
}
...
static void test_get_host_id(void)
{
	...
	err = nvme_get_features_host_id(
		test_link, TEST_SEL, false, sizeof(hostid), get_hostid);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
...
static void test_get_host_id_extended(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_features_host_id(
		test_link, TEST_SEL, true, sizeof(hostid), get_hostid);
	end_mock_cmds();
	check(err == 0, "get features returned error %d", err);
	cmp(get_hostid, hostid, sizeof(hostid), "incorrect host identifier");
}
...
static void test_set_resv_mask(void)
{
	...
	err = nvme_set_features_resv_mask(
		test_link, TEST_NSID, true, MASK, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_resv_persist(void)
{
	...
	err = nvme_set_features_resv_persist(
		test_link, TEST_NSID, false, true, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_write_protect(void)
{
	...
	err = nvme_set_features_write_protect(
		test_link, TEST_NSID, save, STATE, &result);
	end_mock_cmds();
	check(err == 0, "set features returned error %d", err);
	check(result == TEST_RESULT,
	      "got result %" PRIu32 ", expected %" PRIu32, result, TEST_RESULT);
...
static void test_set_status_code_error(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_async_event(test_link, false, EVENTS, &result);
	end_mock_cmds();
...
static void test_set_kernel_error(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_set_features_resv_mask(
		test_link, TEST_NSID, false, MASK, &result);
	end_mock_cmds();
...
static void test_lm_set_features_ctrl_data_queue(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_lm_set_features_ctrl_data_queue(test_link, TEST_CDQID, hp, tpt,
						   etpt, &result);
	end_mock_cmds();

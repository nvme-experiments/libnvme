...
static void test_format_nvm(void)
{
	...
	err = nvme_format_nvm(test_link, lbafl, mset, pi, pil,
			      ses, lbafu, nsid, &result);
	end_mock_cmds();
...
static void test_ns_mgmt(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_ns_mgmt(test_link, NVME_NS_MGMT_SEL_CREATE, TEST_CSI,
			   &data, TEST_NSID, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
...
static void test_ns_attach(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_ns_attach(test_link, sel, &ctrlist, nsid, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
...
static void test_security_send(void)
{
	...
	memcpy(&data, &expected_data, sizeof(expected_data));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_security_send(test_link, nssf, spsp0, spsp1, secp, tl,
				 nsid, &expected_data, data_len, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
	cmp(&data, &expected_data, sizeof(data), "incorrect data");
...
static void test_resv_register(void)
{
	...
	set_mock_io_cmds(&mock_io_cmd, 1);
	err = nvme_resv_register(test_link, &args);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
}
...
static void test_resv_release(void)
{
	...
	set_mock_io_cmds(&mock_io_cmd, 1);
	err = nvme_resv_release(test_link, &args);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
}
...
static void test_lm_cdq(void)
{
	...
	memcpy(&data, &expected_data, sizeof(expected_data));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_lm_cdq(test_link, &args);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
	cmp(&data, &expected_data, sizeof(data), "incorrect data");
...
static void test_lm_migration_send(void)
{
	...
	memcpy(&data, &expected_data, sizeof(expected_data));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_lm_migration_send(test_link, &args);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
	cmp(&data, &expected_data, sizeof(data), "incorrect data");

...
static void test_security_receive(void)
{
	...
	arbitrary(&expected_data, sizeof(expected_data));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_security_receive(test_link, TEST_NSID, al, nssf, spsp0, spsp1, secp, &data,
				    sizeof(data), &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
...
static void test_get_lba_status(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_get_lba_status(test_link, TEST_NSID, slba, atype, rl, lbas, mndw, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
...
static void test_directive_send(void)
{
	...
	memcpy(&data, &expected_data, sizeof(expected_data));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_directive_send(test_link, TEST_NSID, doper, dtype, cdw12, dspec, &expected_data,
				  data_len, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned wrong result");
...
static void test_directive_recv(void)
{
	...
	arbitrary(&expected_data, sizeof(expected_data));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_directive_recv(test_link, TEST_NSID, doper, dtype, cdw12, dspec, &data, data_len,
				  &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned wrong result");
...
static void test_capacity_mgmt(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_capacity_mgmt(test_link, cdw11, cdw12, element_id, op, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == expected_result, "wrong result");
}
...
static void test_lockdown(void)
{
	...
	struct mock_cmd mock_admin_cmd = {
		...
		.cdw10 = ofi << 8 | (ifc & 0x3) << 5 | (prhbt & 0x1) << 4 | (scp & 0xF),
		.cdw14 = (__u32)(uuidx & 0x3F),
		.result = expected_result,
	};

	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_lockdown(test_link, scp, prhbt, ifc, ofi, uuidx, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == expected_result, "wrong result");
}
...
{
	enum nvme_sanitize_sanact sanact = NVME_SANITIZE_SANACT_START_CRYPTO_ERASE;
	__u32 expected_result = 0x45, result = 0;
	...
	struct mock_cmd mock_admin_cmd = {
		...
		.cdw10 = sanact | (!!ause << 3) | (owpass << 4) | (!!oipbp << 8) | (!!nodas << 9),
		.cdw11 = ovrpat,
		.result = expected_result,
	};

	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_sanitize_nvm(test_link, sanact, ovrpat, ause, owpass, oipbp, nodas, emvs,
				&result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == expected_result, "wrong result");
...
{
	enum nvme_dst_stc stc = NVME_DST_STC_ABORT;
	__u32 expected_result = 0x45, result = 0;
	__u32 nsid = TEST_NSID;
	int err;
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_dev_self_test(test_link, nsid, stc, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == expected_result, "wrong result");
}
...
static void test_virtual_mgmt(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_virtual_mgmt(test_link, act, rt, cntlid, nr, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == expected_result, "wrong result");
}
...
static void test_copy(void)
{
	...
	set_mock_io_cmds(&mock_io_cmd, 1);
	err = nvme_copy(test_link, TEST_NSID, sdlba, lr, fua, dspec, lbatm, lbat, prinfor, prinfow,
			dtype, format, ilbrt_u64, copy, nr, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
}
...
static void test_resv_acquire(void)
{
	...
	set_mock_io_cmds(&mock_io_cmd, 1);
	err = nvme_resv_acquire(test_link, TEST_NSID, 0, 0, rtype, racqa, iekey, &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
}
...
static void test_resv_register(void)
{
	...
	set_mock_io_cmds(&mock_io_cmd, 1);
	err = nvme_resv_register(test_link, TEST_NSID, 0xffffffffffffffff, 0, rrega, cptpl, iekey,
				 &result);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
...
static void test_io_mgmt_recv(void)
{
	...
	arbitrary(&expected_data, sizeof(expected_data));
	set_mock_io_cmds(&mock_io_cmd, 1);
	err = nvme_io_mgmt_recv(test_link, TEST_NSID, mos, mo, &data, data_len, NULL);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	cmp(&data, &expected_data, sizeof(data), "incorrect data");
}
...
static void test_lm_cdq(void)
{
	...
	memcpy(&data, &expected_data, sizeof(expected_data));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_lm_cdq(test_link, mos, 0x2, sel, &data, sz, &cdqid, NULL);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
	cmp(&data, &expected_data, sizeof(data), "incorrect data");
...
static void test_lm_track_send(void)
{
	...
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_lm_track_send(test_link, mos, cdqid, sel, NULL);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
}
...
static void test_lm_migration_send(void)
{
	...
	memcpy(&data, &expected_data, sizeof(expected_data));
	set_mock_admin_cmds(&mock_admin_cmd, 1);
	err = nvme_lm_migration_send(test_link, offset, mos, cntlid, 0x3, sel, uidx, 0x5, 0x6, 0x7,
				     true, &expected_data, sizeof(expected_data), NULL);
	end_mock_cmds();
	check(err == 0, "returned error %d", err);
	check(result == 0, "returned result %u", result);
	cmp(&data, &expected_data, sizeof(data), "incorrect data");

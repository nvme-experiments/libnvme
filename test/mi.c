...
static void test_ctrl_lifetime(nvme_mi_ep_t ep)
{
	...
	l1 = nvme_mi_init_link(ep, 1);
	count = count_ep_controllers(ep);
	assert(count == 1);

	l2 = nvme_mi_init_link(ep, 2);
	count = count_ep_controllers(ep);
	assert(count == 2);
...
static void test_scan_ctrl_list(nvme_mi_ep_t ep)
{
	...
	link = nvme_mi_first_link(ep);
	assert(link);
	assert(link->id == 1);

	link = nvme_mi_next_link(ep, link);
	assert(link);
	assert(link->id == 4);
...
static void test_admin_invalid_formats(nvme_mi_ep_t ep)
{
	...
	/* unaligned req size */
	len = 0;
	rc = nvme_mi_admin_xfer(link, &req.hdr, 1, &resp, 0, &len);
	assert(rc != 0);

	/* unaligned resp size */
	len = 1;
	rc = nvme_mi_admin_xfer(link, &req.hdr, 0, &resp, 0, &len);
	assert(rc != 0);

	/* unaligned resp offset */
	len = 4;
	rc = nvme_mi_admin_xfer(link, &req.hdr, 0, &resp, 1, &len);
	assert(rc != 0);
	...
	len = 4;
	rc = nvme_mi_admin_xfer(link, &req.hdr, 0, &resp, (off_t)1 << 32, &len);
	assert(rc != 0);

	/* resp offset with no len */
	len = 0;
	rc = nvme_mi_admin_xfer(link, &req.hdr, 0, &resp, 4, &len);
	assert(rc != 0);

	/* req and resp payloads */
	len = 4;
	rc = nvme_mi_admin_xfer(link, &req.hdr, 4, &resp, 0, &len);
	assert(rc != 0);
}
...
static void test_get_features_nodata(nvme_mi_ep_t ep)
{
	...
	rc = nvme_get_features(link, NVME_NSID_NONE, 0, NVME_FEAT_FID_ARBITRATION,
			       0, NVME_UUID_NONE, NULL, 0, &res);
	assert(rc == 0);
...
static void test_get_features_data(nvme_mi_ep_t ep)
{
	...
	rc = nvme_get_features(link, NVME_NSID_NONE, 0, NVME_FEAT_FID_TIMESTAMP,
			       0, NVME_UUID_NONE, &tstamp, sizeof(tstamp), &res);
	assert(rc == 0);
...
static void test_set_features(nvme_mi_ep_t ep)
{
	...
	rc = nvme_set_features(link, 0, 0, 0, 0, 0, NVME_FEAT_FID_TIMESTAMP,
			       1, 0, &tstamp, sizeof(tstamp), &res);
	assert(rc == 0);
...
static void test_admin_id_alloc_ns_list(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_identify_allocated_ns_list(link, 1, &list);
	assert(!rc);
...
static void test_admin_id_active_ns_list(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_identify_active_ns_list(link, 1, &list);
	assert(!rc);
...
static void test_admin_id_alloc_ns(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_identify_allocated_ns(link, 1, &id);
	assert(!rc);
...
static void test_admin_id_nsid_ctrl_list(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_identify_nsid_ctrl_list(link, 0x01020304, 5, &list);
	assert(!rc);
}
...
static void test_admin_id_secondary_ctrl_list(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_identify_secondary_ctrl_list(link, 5, &list);
	assert(!rc);
}
...
static void test_admin_ns_mgmt_delete(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_ns_mgmt_delete(link, 0x05060708);
	assert(!rc);
}
...
static void test_admin_ns_attach(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_ns_attach_ctrls(link, 0x02030405, &list);
	assert(!rc);
}
...
static void test_admin_ns_detach(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_ns_detach_ctrls(link, 0x02030405, &list);
	assert(!rc);
}
...
static void test_admin_fw_commit(struct nvme_mi_ep *ep)
{
	...
	info.slot = 0;
	info.action = 0;
	rc = nvme_fw_commit(link, info.slot, info.action, info.bpid, NULL);
	assert(!rc);
				      ...
				      struct nvme_mi_resp *resp, void *data)
{
	...
	assert(((rq_hdr[45] >> 1) & 0x1) == info->nodas);
...
static void test_admin_sanitize_nvm(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_sanitize_nvm(link, info.sanact, info.ovrpat, info.ause, info.owpass, info.oipbp,
			       info.nodas, info.emvs, NULL);
	assert(!rc);
...
static void test_admin_dlen_doff_req(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_mi_admin_xfer(link, &admin_req.hdr, sizeof(admin_req.data),
...
static void test_admin_dlen_doff_resp(struct nvme_mi_ep *ep)
{
	...
	rc = nvme_mi_admin_xfer(link, &admin_req, 0, &admin_resp.hdr, 0,
				&resp_sz);

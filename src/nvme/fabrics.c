		  ...
		  const struct nvme_fabrics_config *cfg)
{
	...
	if (traddr_is_hostname(h->r, c)) {
		...
		if (hostname2traddr(h->r, traddr, &c->traddr)) {
			c->traddr = traddr;
			    ...
			    nvme_ctrl_t *cp)
{
	...
	ret = nvme_create_ctrl(h->r, e->subnqn, transport, traddr,
			      ...
			      struct nvmf_discovery_log **logp)
{
	...
	err = nvme_get_log(l, false, args->lsp, NVME_LOG_LID_DISCOVER,
		NVME_LOG_LSI_NONE, NVME_CSI_NVM, false, NVME_UUID_NONE,
		NVME_NSID_NONE, NVME_LOG_PAGE_PDU_SIZE, DISCOVERY_HEADER_LEN,
		0, log, NULL);
	if (err) {
		nvme_msg(l->root, LOG_INFO,
			 "%s: discover try %d/%d failed, error %d\n",
			 name, retries, args->max_retries, err);
		goto out_free_log;
	}
	...
	do {
		...
		nvme_msg(l->root, LOG_DEBUG,
			 "%s: get %" PRIu64 " records (genctr %" PRIu64 ")\n",
			 name, numrec, genctr);

		err = nvme_get_log(l, false, args->lsp, NVME_LOG_LID_DISCOVER,
			NVME_LOG_LSI_NONE, NVME_CSI_NVM, false, NVME_UUID_NONE,
			NVME_NSID_NONE, NVME_LOG_PAGE_PDU_SIZE, entries_size,
			sizeof(*log), log->entries, NULL);
		if (err) {
			nvme_msg(l->root, LOG_INFO,
				 "%s: discover try %d/%d failed, error %d\n",
				 name, retries, args->max_retries, err);
	...
	if (genctr != le64_to_cpu(log->genctr)) {
		nvme_msg(l->root, LOG_INFO, "%s: discover genctr mismatch\n", name);
		err = -EAGAIN;

...
int nvme_mi_scan_ep(nvme_mi_ep_t ep, bool force_rescan)
{
	...
	for (i = 0; i < n_ctrl; i++) {
		...
		hdl = nvme_mi_init_transport_handle(ep, id);
		if (!hdl)
			break;
	}
...
int nvme_mi_async_read(nvme_mi_ep_t ep, struct nvme_mi_resp *resp)
{
	...
	if (ep->transport->mic_enabled) {
		...
		if (rc) {
			nvme_msg(ep->ctx, LOG_WARNING, "crc mismatch\n");
			return -EBADMSG;
		}
	}
		       ...
		       size_t *resp_data_size)
{
	...
	admin_req->ctrl_id = cpu_to_le16(hdl->id);
	memset(&req, 0, sizeof(req));
...
nvme_mi_ep_t nvme_mi_next_endpoint(struct nvme_global_ctx *ctx, nvme_mi_ep_t ep)
{
...
{
	return list_top(&ep->controllers, struct nvme_transport_handle, ep_entry);
}

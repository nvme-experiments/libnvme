			   ...
			   size_t *size)
{
	...
	*size = 0;
	...
	if (ctrl) {
		err = nvme_get_log_telemetry_ctrl(l, true, 0, xfer, log);
		...
		if (create)
			...
			err = nvme_get_log_telemetry_host(l, 0, xfer, log);
	}
...
int nvme_get_lba_status_log(nvme_link_t l, bool rae, struct nvme_lba_status_log **log)
{
	...
	err = nvme_get_log_lba_status(l, true, 0, sizeof(*buf), buf);
	if (err) {
		*log = NULL;
		return err;
	}
			      ...
			      __u16 *ctrlist, bool attach)
{
	...
	return nvme_ns_attach(l, sel, &cntlist, nsid, NULL);
}

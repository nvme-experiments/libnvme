		     ...
		     nvme_ctrl_t *cp)
{
	...
	if (host_traddr) {
		if (traddr_is_hostname(transport, host_traddr))
			hostname2traddr(r, host_traddr, &c->cfg.host_traddr);
...
static int nvme_ctrl_lookup_phy_slot(nvme_root_t r, const char *address, char **slot)
{
				 ...
				 const char *name)
{
	...
	nvme_ctrl_lookup_phy_slot(r, c->address, &c->phy_slot);
...
int nvme_init_ctrl(nvme_host_t h, nvme_ctrl_t c, int instance)
{
	...
	ret = nvme_ctrl_lookup_subsystem_name(h->r, name, &subsys_name);
	if (ret) {
...
int nvme_scan_ctrl(nvme_root_t r, const char *name, nvme_ctrl_t *cp)
{
	...
	ret = nvme_ctrl_lookup_subsystem_name(r, name, &subsysname);
	if (ret) {
		nvme_msg(r, LOG_DEBUG,
...
int nvme_ns_verify(nvme_ns_t n, off_t offset, size_t count)
{
	...
	return nvme_verify(nvme_ns_get_link(n), nvme_ns_get_nsid(n), slba,
			   nlb, 0,
			   0,
			   false, 0, 0, 0, 0,
			   0, 0,
...
int nvme_ns_write_uncorrectable(nvme_ns_t n, off_t offset, size_t count)
{
	...
	return nvme_write_uncorrectable(nvme_ns_get_link(n), nvme_ns_get_nsid(n), slba,
					nlb, 0,
					0,
					NULL);
}
...
int nvme_ns_write_zeros(nvme_ns_t n, off_t offset, size_t count)
{
	...
	return nvme_write_zeros(nvme_ns_get_link(n), nvme_ns_get_nsid(n), slba,
				nlb, 0,
				0, 0, 0,
				false, 0, 0, 0, 0,
				0, 0,
...
int nvme_ns_write(nvme_ns_t n, void *buf, off_t offset, size_t count)
{
	...
	return nvme_write(nvme_ns_get_link(n), nvme_ns_get_nsid(n), slba,
			  nlb, 0,
			  0, 0, 0,
			  false, 0, 0, 0, 0,
			  0, 0,
...
int nvme_ns_read(nvme_ns_t n, void *buf, off_t offset, size_t count)
{
	...
	return nvme_read(nvme_ns_get_link(n), nvme_ns_get_nsid(n), slba,
			 nlb, 0, 0, 0,
			 false, 0, 0, 0, 0,
			 0, 0,
			 NULL, 0,
...
int nvme_ns_compare(nvme_ns_t n, void *buf, off_t offset, size_t count)
{
	...
	return nvme_compare(nvme_ns_get_link(n), nvme_ns_get_nsid(n), slba,
			    nlb, 0,
			    0,
			    false, 0, 0, 0, 0,
			    0, 0,
